...
28. [ ] [`28-declare-mount-in-root-manifest.md`](tasks/28-declare-mount-in-root-manifest.md) — feat: `generate.sh` writes its `tasks` and `inbox` keys into **`discovery.json`** at the target repo root, so consumers stop probing for the mount. 27 makes an install's *contents* legible; this makes its *location* legible, and a consumer that cannot find the mount never reaches the lister. Requested by `dev-workspace` 2026-08-06 while planning inter-repo mail, which needs the same fact `replan.sh:53-62` currently guesses at. Cannot live in `task-config.sh` (task 26) — that file is inside the directory being located. **One shared file, not one per generator**: `create-context-hygiene` task `13` writes a `context-hygiene` key into the same file, so the name and the merge rule must be agreed with that repo before either writes code — a generator that rewrites the file wholesale erases the other's key. **create-project-system-owned.**
