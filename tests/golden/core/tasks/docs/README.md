...
All scripts are in `tasks/scripts/` and should be run from the
**repo root**.
...
tasks/scripts/new-user-task.sh --epic main --folder draft \
    --name my-project --worktree task-tooling --tags "docs, search"

# Create a human-owned subtask (review, planning step, etc.)
tasks/scripts/new-user-subtask.sh --epic main --folder in-progress \
    --parent my-project --name design-review

# Create a pipeline entry point (build-N)
tasks/scripts/new-pipeline-subtask.sh --epic main --folder in-progress \
    --parent my-project --name build-1

# Move a task (and all its subtasks) to a different status
tasks/scripts/move-task.sh --epic main --name my-task \
    --from draft --to backlog

# Mark a top-level task complete (moves to complete/)
tasks/scripts/complete-task.sh --epic main --folder in-progress --name my-task

# Mark a subtask complete (updates checkbox and Status field)
tasks/scripts/complete-task.sh --epic main --folder in-progress \
    --parent my-task --name my-subtask

# Undo either
tasks/scripts/complete-task.sh --epic main --folder in-progress --name my-task --undo
tasks/scripts/complete-task.sh --epic main --folder in-progress \
    --parent my-task --name my-subtask --undo

# Print a task's README to stdout
tasks/scripts/show-task.sh --epic main --folder in-progress --name my-task

# Change Tags or Priority on an existing task (never hand-edit the table)
tasks/scripts/set-field.sh --epic main --folder backlog --name my-task \
    --field Tags --value "docs, search"
tasks/scripts/set-field.sh --epic main --folder backlog --name my-task \
    --field Priority --value HIGH

# Soft-delete a task (hides directory, removes from parent README)
tasks/scripts/delete-task.sh --epic main --folder draft --name my-task

# Restore a soft-deleted task
tasks/scripts/restore-task.sh --epic main --folder draft --name my-task

# Mark a subtask as wont-do (sets Status, removes from parent list, keeps directory)
tasks/scripts/wont-do-subtask.sh --epic main --folder in-progress \
    --parent my-task --name my-subtask

# List incomplete tasks in an epic
tasks/scripts/list-tasks.sh --epic main --folder draft
tasks/scripts/list-tasks.sh --epic main --folder backlog
tasks/scripts/list-tasks.sh --epic main --folder in-progress

# List all tasks including completed
tasks/scripts/list-tasks.sh --epic main --all

# List tasks with subtask depth
tasks/scripts/list-tasks.sh --epic main --folder in-progress --depth 2

# Filter by tag
tasks/scripts/list-tasks.sh --epic main --tag backend --depth 2 --all

# Filter by Worktree (which files a task touches — see worktrees.md)
tasks/scripts/list-tasks.sh --epic main --folder backlog --worktree task-tooling
...
tasks/scripts/insert-subtask.sh --epic main --folder in-progress \
